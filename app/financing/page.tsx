...
export default async function FinancingPage() {
  ...
  let byStore: Awaited<ReturnType<typeof getFinancingByStore>>;
  try {
    [divisions, financing, byStore] = await Promise.all([
      getDivisions(),
      getFinancing(),
      getFinancingByStore(),
    ]);
  ...
  return (
    <>
      ...
      <main className="wrap">
        ...
        {latest.map((p) => {
          const rows = storeRowsFor(byStore, p.provider);
          if (!rows.length) return null;
          return (
            <section key={p.provider}>
              <div className="sec-head">
                <h2>{p.provider} — by store</h2>
                <p>
                  {rows.length} store{rows.length === 1 ? "" : "s"} with
                  activity in this window. These rows sum to the{" "}
                  {p.provider} line in the comparison above. A store with no
                  activity does not appear.
                </p>
              </div>
              <FinancingByStore rows={rows} />
            </section>
          );
        })}
        ...
        <div className="provenance">
          ...
          as “—” otherwise rather than being estimated. Store-level rows are
          keyed on the provider's own identifier — Affirm's{" "}
          <code>merchant_ari</code>, Snap's store number, CareCredit's
          merchant number — never on a free-text store field. Page data
          refreshes hourly.
        </div>
      </main>
    </>
