...
Percent fields are stored as plain numbers scaled 0–100 (`63.52` means 63.52%),
matching the import CSVs. Currency fields are plain numbers. If someone converts
a percent column to Airtable's native Percent type, it will display as 6352% —
...
5. **Breakeven build-up** — the variable rates and fixed cost behind each
   store's target.
6. **Corporate overhead** — what the group must clear on top of store targets.
7. **Cost standards** — the target operating model.
