  ...
  return recs
    .map((r) => {
      ...
      return {
        ...
        // Net Volume is net of BOTH refunds and merchant fees -- the cash the
        // provider actually funded. The fallback must use the same definition
        // as the column, or a row with the cell left blank would silently sit
        // on a different basis from every other row and the column would not
        // total. See the field description on Financing in Airtable.
        netVolume:
          num(f["Net Volume"]) ?? grossVolume - refunds - (merchantFees ?? 0),
        merchantFees,
        effectiveRate,
        avgTicket: num(f["Avg Ticket"]),
        notes: f["Notes"] ?? "",
