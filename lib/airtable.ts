...
/**
 * Net sales per division per week across the window, oldest week first --
 * the series behind the trend chart.
 *
 * Every division gets a point for every week in the window. A week with no
 * rows for a division is `null`, not 0: a division that reported nothing is a
 * gap in the line, whereas 0 would draw a plunge to the floor and read as a
 * catastrophic week. Divisions with no rows at all in the window are dropped
 * entirely rather than drawn as a flat empty line.
 */
export type DivisionSeriesPoint = {
  weekEnding: string;
  weekLabel: string;
...
export type DivisionSeries = {
  ...
  division: string;
  slug: string;
  points: DivisionSeriesPoint[];
  /** Sum across the window, for ordering the legend and the table. */
  total: number;
  /** First and last non-null points, for the start-to-end change. */
...
export function divisionSeries(
  ...
  for (const w of weeks) {
    if (!isActual(w) || !w.divisionId) continue;
    if (!ordered.includes(w.weekEnding)) continue;
    const list = byDivision.get(w.divisionId) ?? [];
    list.push(w);
    byDivision.set(w.divisionId, list);
  }
  ...
  return divisions
    ...
    .map((d) => {
      const rows = byDivision.get(d.id) ?? [];
      const points: DivisionSeriesPoint[] = ordered.map((we) => {
        const forWeek = rows.filter((r) => r.weekEnding === we);
        return {
          weekEnding: we,
          weekLabel: labels.get(we) ?? we,
          netSales: forWeek.length
            ? forWeek.reduce((a, r) => a + r.netSales, 0)
            : null,
          units: forWeek.length
            ? forWeek.reduce((a, r) => a + (r.units ?? 0), 0)
            : null,
        };
      });
      const real = points.filter((p) => p.netSales !== null);
      const first = real[0]?.netSales ?? null;
      const last = real[real.length - 1]?.netSales ?? null;
      return {
        divisionId: d.id,
        division: d.name,
        slug: d.slug,
        points,
        total: real.reduce((a, p) => a + (p.netSales ?? 0), 0),
        changePct:
